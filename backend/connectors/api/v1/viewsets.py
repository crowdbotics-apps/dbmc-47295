...
class testconnectorViewSet(BaseConnectorViewSet):
    PASSWORD = settings.CONNECTOR_TEST_CONNECTOR_PASSWORD
    USERNAME = settings.CONNECTOR_TEST_CONNECTOR_USERNAME
    BASE_URL = "https://192.168.56.3/"
    AUTH_TYPE = "basic"
    IDENTIFIER = "CONNECTOR_TEST_CONNECTOR"

    @action(detail=False, methods=["post"], url_path="add_computer")
    def add_computer(self, request, *args, **kwargs):
        params = request.query_params
        payload = request.data
        data = self._api_call(
            "/add_computer", request_type="post", payload=payload, params=params
        )
